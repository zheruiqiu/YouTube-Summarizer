...
const AI_MODELS = {
  gemini: {
    ...
    async generateContent(prompt: string) {
      ...
      const enhancedPrompt = `IMPORTANT INSTRUCTION: Base your summary STRICTLY on the provided content - do not include any imagined, fabricated, or speculative information that is not present in the original content. Be aware that the content may come from speech recognition of video/audio, so some words or phrases might be incorrectly transcribed - use your best judgment when encountering unclear or nonsensical passages.\n\n${prompt}`;
  ...
  (async () => {
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        IMPORTANT: Base your summary STRICTLY on the provided content - do not include any imagined, fabricated, or speculative information that is not present in the original content. Be aware that the content may come from speech recognition of video/audio, so some words or phrases might be incorrectly transcribed - use your best judgment when encountering unclear or nonsensical passages.
